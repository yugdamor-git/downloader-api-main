...
async function fetchThumbnailData(url) {
  ...
  const response = await fetch(`https://api.savetube.me/backend/api/v1/thumbnails?url=${url}`, {
    headers: {
...
async function fetchData(url) {
  ...
  const response = await fetch(`https://api.savetube.me/backend/api/v1/info?url=${url}`);
